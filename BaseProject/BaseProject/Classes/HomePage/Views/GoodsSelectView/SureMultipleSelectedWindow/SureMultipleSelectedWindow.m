...
#define ITEM_SIZE ((WINDOW_WIDTH / 4) - 15)
...
+ (void)showWindowWithTitle:(NSString*)title
  ...
  defaultSelectedConditions:(NSArray*)selectedConditions
    ...
    window.collectionView.frame = CGRectMake(0, WINDOW_HEIGHT - (rows * 40 + ((rows - 1) * 5) + 100), WINDOW_WIDTH,  rows * 40 + ((rows - 1) * 5) + 100);
...
- (UICollectionView*)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerNib:[UINib nibWithNibName:@"SureFooterCollectionReusableView" bundle:nil] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"FOOTER"];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (self.selectedBlock) {
        self.selectedBlock(self.selectedArr);
    }
}
